...
## Overview
A comprehensive client-server competitive programming platform built in C. The platform supports user authentication, problem solving, real time 1v1 duels, an Elo rating system, and a robust backend. The project heavily utilizes core Operating System concepts such as socket programming, multi threading, concurrency control, inter process communication, file locking, and secure sandboxing to execute user submitted code safely.

## Key OS Concepts Demonstrated
- **Concurrency & Multithreading**: Extensive use of Pthreads (One thread between server and each client, and one incase of duel between 2 users), Mutexes (`activeUserList`, `activeChallengesList`), and Semaphores (`job_queue`) to handle multiple concurrent clients and restrict the maximum number of simultaneous execution jobs.
...
- **`genericDatabase.c / .h`**: An generic file based database. It avoids raw reads/writes by establishing `fcntl()` advisory locks on database files to prevent corruption during concurrent accesses. It uses function pointers (`MatchFunction`) for searching records, and supports pagination, updating, inserting, and deleting records of arbitrary sizes, effectively making it a highly modular code.
- **`queryDatabase.c / .h`**: The concrete data access layer built over `genericDatabase.c`. It manages `users.dat` (user auth and roles), `problems.dat` (challenges), and `leaderboard.dat` (Elo ratings), Also provides the specific `mathFunction` and `compareFunction` for comparing and sorting records.
...
- **`client.c`**: A command-line interface application that connects to the server via TCP sockets. 
  ...
  - **Data Encoding**: Transmits user commands and data structures securely inside JSON packets. 
  - **Remote Execution Interface**: Collects the absolute path to a user's local `.c` solution file, efficiently streams it to the server using the `sendFile()` utility, and waits for the server's judgment (Accepted, Wrong Answer, Compilation Error, Runtime Error).
...
- **`seccomp-bpf.h`**: A third party (external) header file packed with BPF (Berkeley Packet Filter) macro definitions. It is critical for defining the secure execution wrapper policy used by the server's sandbox (whitelisting calls like `read`, `write`, `exit`, and dropping `execve` or unauthorized filesystem accesses).
...
*Note: The platform connects to `INADDR_ANY` by default when run locally. Use `#define ONLINE_VM` within `client.c` to bind to the Azure remote server configuration if needed (So that different devices across the world can connect).*
